...
:::message
本記事の数値は大会進行中のスナップショットに基づきます。優勝確率・乖離テーブル・精度検証などの定量データは **2026-06-20 時点（グループステージ第30試合終了）** のスナップショットを基準とし、一部の途中経過（グループ突破の深掘りなど）は **6/21 JST 時点** の値を併記しています。確率は大会の進行とともに変化するため、最新値は本番サイトをご参照ください。
:::
...
- **評価軸**: 確率予測なので「当たった/外れた」では測りません。**Brier score**・**log loss**・**calibration（予測確率と実現頻度の一致度）**で評価します（後述の「モデル精度の検証」）。
...
後述の乖離分析で「モデル」と呼ぶのは前者（シム mix）の優勝確率です。
...
なぜこの配分なのでしょうか。Dixon-Coles と オッズに厚く、Elo を軽くしています。直感的には「スコアの構造（DC）」と「市場の総意（オッズ）」を二本柱にし、Elo は直近の勢いを補助的に効かせる役回りです。現行の 0.4 / 0.2 / 0.4 は過去大会で最適化した既定値で、Brier / log loss のバックテストで再最適化できる仕組みになっています。
...
なおオッズは単一ブック（FanDuel）の上位13チームのみ収録で、下位ロングショットは未掲載（＝市場確率なし）です。乖離分析もオッズのあるチームに限られる点に注意してください（複数ブックの平均化は「今後拡張したいこと」で触れます）。

### 3者をなぜ混ぜるのか

3つの長所と短所は補い合う関係にあります。**Dixon-Coles** はスコアの構造（攻守の質）を捉えますが、直近の勢いには鈍いモデルです。**Elo** は勢いに敏感ですが、相手の質や試合内容を1次元に潰してしまいます。**オッズ** は最も情報量が多い（移籍・負傷・主観すべてを織り込む）一方で、人気バイアスや市場心理が混ざります。1つが外す局面を別の1つが拾う——これがブレンドの狙いです。実際に単体とブレンドのどちらが当たっているかは、後述の「モデル精度の検証」で確かめます。
...
結局のところ、**モデルと市場のどちらが正しいかは大会が答えを出します**。この乖離テーブルは、その答え合わせの「賭け表」でもあるわけです。
...
**ブレンドの土台である mix が、Dixon-Coles 単体に Brier・log loss の両方で負けています**。「混ぜれば強くなる」という期待に反し、この小標本（28試合）では DC 単体がいちばん良い結果でした。Elo を混ぜたことがむしろスコアをわずかに濁らせた格好です。もっとも28試合は統計的にまだ薄く、ノックアウトに入ればまた変わりうるでしょう。それでも「アンサンブルは常に最強」ではないことを、実データが突きつけてくる結果となりました。
...
- **ブレンドは万能ではない**: 「モデル精度の検証」のとおり、現時点では DC 単体がベストです。ブレンド重みは過去大会で最適化したものですが、**今大会の実データで再最適化する余地が大きい**と感じます。特に Elo の 0.2 は、このサンプルを見る限り下げる方向の検討に値するでしょう。
...
個人的にいちばん面白いと感じているのは、大会がまだグループステージの途中（本稿執筆時点）だという点です。乖離テーブルはいまのところ「仮説の束」にすぎず、アルゼンチン強気もフランス弱気も答え合わせはこれからです。ノックアウトに入ってサンプルが増えれば、DC 単体優位という精度検証の結論も、ブレンド重みの妥当性も、おそらく書き換わります。本記事は「完成した予測」ではなく「進行中の検証ログ」だと捉えていただくのが正確で、大会終了後にあらためて答え合わせの追記をしたいと考えています。
...
- **ブレンド重みの再最適化**: 精度検証の結果を受け、今大会データでバックテストを回し、Elo の比重を見直します。
- **$\xi$・K値の安定性検証**: 過去複数大会でのバックテストを自動化し、ハイパラの頑健性を確かめたいと考えています。

直近で優先したいのは、ノックアウトに入ってサンプルが増えた段階での精度検証のやり直しと、それを踏まえたブレンド重みの再最適化です。大会が進むほど確率分布は鋭くなるので、各節目のスナップショットを残しておき、「どの段階でモデルと市場の答えが確定したか」を時系列で振り返れるようにしたいと考えています。
